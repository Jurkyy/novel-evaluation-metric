...
def run(model_name, metrics_names, loss_func, nb_epochs, batch_size, learning_rate, reshape_x_attack, early_stopping,
    ...
    # Our folders
    root = "./datasets/ASCAD_50/"
    ASCAD_data_folder = root + "training_data/"
    ASCAD_trained_models_folder = "results/ASCAD_50/trained_models/"
    history_folder = "results/ASCAD_50/training_history/"
    predictions_folder = "results/ASCAD_50/model_predictions/"
    ...
    (X_profiling, Y_profiling), (X_attack, Y_attack), (plt_profiling, plt_attack) = load_ascad(
        ASCAD_data_folder + "ASCAD_desync50.h5", load_metadata=True)
    ...
    model = getattr(importer.import_module("models"), model_name)(loss_function=loss_function, metrics=metrics,
                                                                  ...
                                                                  classes=9)
    model_name = "ASCAD50_{:s}_e{:d}_b{:d}.h5".format(loss_func, nb_epochs, batch_size)
    ...
    exploit_pred = importer.import_module("datasets.ASCAD_50.exploit_pred")
    avg_rank = exploit_pred.perform_attacks(nb_traces_attacks, predictions, nb_attacks, plt=plt_attack, key=real_key, byte=2,
                               filename=model_name)

    np.save("results/ASCAD_50/plot_data/" + model_name.replace('.h5', '.npy'), avg_rank)
